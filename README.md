...
- **Dagger CI/CD**: Branch-based automations (no static YAML pipelines)
- **CDK8s Infrastructure**: All Kubernetes manifests generated from TypeScript (zero static YAML)
- **Crossplane GitOps**: Multi-cloud infrastructure orchestration with Server-Side Apply
...
## No Static YAML Policy

This repository follows a **zero static YAML** policy for Kubernetes manifests:

- All K8s manifests are generated by **CDK8s** from TypeScript constructs
- Infrastructure definitions live in `infra/src/constructs/`
- Generated manifests output to `infra/dist/` (gitignored or committed for GitOps)
- The only YAML files are:
  - `.github/workflows/*.yml` - GitHub Actions (required by GitHub)
  - `infra/cdk8s.yaml` - CDK8s configuration
  - `examples/*.yaml` - Example claim manifests for documentation
...
```
lornu.ai/
├── services/          # Rust services (@engine-team)
│   ├── engine/        # Core engine
│   ├── gateway/       # API gateway
│   └── agent-worker/  # Agent execution runtime
├── infra/             # Infrastructure (TypeScript/CDK8s) (@infra-ops)
│   ├── src/constructs/  # CDK8s constructs (generates all K8s YAML)
│   ├── main.ts          # CDK8s app entry point
│   ├── cdk8s.yaml       # CDK8s configuration
│   └── dist/            # Generated manifests (gitignored)
...
Generate Kubernetes manifests from TypeScript:
...
# Synthesize manifests for dev environment
LORNU_ENV=dev bun run main.ts

# Output appears in infra/dist/
ls dist/
```

See [infra/README.md](./infra/README.md) for full infrastructure documentation.
